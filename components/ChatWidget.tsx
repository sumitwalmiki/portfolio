    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <Button
          ...
          className="w-16 h-16 rounded-full gradient-primary text-white shadow-professional-xl card-hover"
        >
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-96 h-[500px] z-50 animate-scale-in">
          <div className="h-full bg-white rounded-2xl shadow-professional-xl border border-slate-200 flex flex-col overflow-hidden">
            {/* Header */}
            <div className="gradient-primary text-white p-6 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-bold text-lg">Sumit Bot</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50">
              {messages.map((message) => (
                <div key={message.id} className={`flex ${message.sender === "user" ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`max-w-[80%] p-4 rounded-2xl shadow-professional ${
                      message.sender === "user"
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-white text-slate-900 p-4 rounded-2xl rounded-bl-md shadow-professional border border-slate-200">
                ...
                <div className="space-y-2">
                  {quickReplies.map((reply) => (
                    <button
                      ...
                      className="block w-full text-left p-3 bg-white hover:bg-slate-100 border border-slate-200 hover:border-slate-300 rounded-xl text-sm text-slate-700 font-medium transition-all duration-200 shadow-professional"
                    >
                      {reply}
                    </button>
                  ))}
